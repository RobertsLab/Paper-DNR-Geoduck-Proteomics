...
# Download DIA Protein abundance report from Owl, or locally if you prefer (it's much faster if you import from a locally saved file)
...
length(unique(DIA.annotated$SPID)) # This is the number of proteins identified that are annotated with a SPID
...
# Don't need to remove any reps, look good!
# DIA.noOuts <- DIA.annotated[!grepl("12|15|17|18|20", DIA.annotated$Replicate),]
# Mean  tech reps across samples 
DIA.mean <- aggregate(Area.adj~Sample+., na.omit(DIA.annotated[,c(3,5,8,9,10,14,19,21,23)]), mean)
# Sum all transitions across proteins for each sample
DIA.mean.protSum <- aggregate(Area.adj~Protein.Name+., DIA.mean[,-3], sum)
#Create new column to assign Region, CI & WB = South, FB & PG = North
DIA.mean.protSum$Region <- DIA.mean.protSum$Site
...
DIA.mean.region <- aggregate(Area.adj~Region+., data=DIA.mean.protSum[,c(-2,-6,-7)], mean)
DIA.mean.region.sd <- aggregate(Area.adj~Region+., data=DIA.mean.protSum[,c(-2,-6,-7)], sd)
