...
# removed ", stringsAsFactors = FALSE" from previous line
# samples.vector <- samples.vector[-121]
SRM.data <- SRMreport
colnames(SRM.data) <- samples.vector
head(SRM.data) #spot check to make sure the column names are correct (row 1 still has rep names)
...
SRM.data.numeric <- SRM.sample.data[-1,] # First, remove row #1 (excess info) & change all area values to numeric, so I can average, etc. I know that my area data is from column 5 to 120 
